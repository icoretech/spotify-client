  ...
  describe 'wrapper methods' do
    it 'calls remove_user_tracks_from_playlist endpoint' do
      expect(client).to receive(:run)
        .with(:delete, '/v1/playlists/p1/tracks', [200], '{"tracks":[{"uri":"spotify:track:1"}]}')
      client.remove_user_tracks_from_playlist('u1', 'p1', [{ uri: 'spotify:track:1' }])
    end

    it 'calls replace_user_tracks_in_playlist endpoint' do
      expect(client).to receive(:run)
        .with(:put, '/v1/playlists/p1/tracks', [200, 201], '{"uris":["spotify:track:1"]}')
    ...
    it 'calls albums endpoint with joined ids' do
      expect(client).to receive(:run).with(:get, '/v1/albums', [200], { :ids => 'a1,a2' })
      client.albums(%w[a1 a2])
    end
    ...
    it 'calls tracks endpoint with joined ids' do
      expect(client).to receive(:run).with(:get, '/v1/tracks', [200], { :ids => 't1,t2' })
      client.tracks(%w[t1 t2])
    end
    ...
    it 'calls artists endpoint with joined ids' do
      expect(client).to receive(:run).with(:get, '/v1/artists', [200], { :ids => 'ar1,ar2' })
      client.artists(%w[ar1 ar2])
    end
    ...
    it 'calls artist_top_tracks endpoint with market' do
      expect(client).to receive(:run).with(:get, '/v1/artists/ar1/top-songs', [200], market: 'IT')
    ...
    it 'calls follow_playlist endpoint' do
      expect(client).to receive(:run).with(:put, '/v1/playlists/p1/followers', [200, 204], { :public => false })
